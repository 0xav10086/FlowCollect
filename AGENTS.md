...
~/FlowCollect/client：唯一合法的客户端 Sidecar (Go语言) 源码及编译输出目录。

~/FlowCollect/server：唯一合法的服务端 (Go语言) 源码及编译输出目录。

~/FlowCollect/smart_spend：纯粹的 Vue 3 前端工程。其 dist 目录只允许存放 HTML/CSS/JS 等前端静态资源，绝不允许污染任何跨平台二进制可执行文件。
...
1. **客户端 (Client - Sidecar 模式)**
   ...
   - client 目录下编译出的程序不是独立给用户双击运行的带界面软件。它是作为一个 Sidecar (旁路守护进程)，被集成到魔改版的宿主客户端（如桌面的 clash-verge-rev、Android 的 box_for_magisk）中。宿主程序在拉起核心（如 Mihomo）时，会在后台静默拉起这个 Sidecar 进程，共同完成代理与数据上报。
